...
function WorkOrdersView() {
  ...
  const handleStartProcedure = async (workOrderId) => {
    try {
      ...
      navigate(`/procedure/${workOrderId}`, { state: { workOrder: workOrders.find(wo => wo.id === workOrderId), agentResult: result } })
    } catch (error) {
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-8">
        <h2 className="text-2xl font-semibold text-slate-900 mb-2">Work Orders</h2>
        <p className="text-slate-600">View and manage active work orders</p>
      </div>
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex-1">
                    <div className="flex items-center space-x-3 mb-2">
                      ...
                      <span className={`px-2.5 py-0.5 text-xs font-medium rounded border ${getStatusColor(workOrder.status)}`}>
                    ...
                    <div className="flex items-center space-x-6 text-sm text-slate-600 mt-3">
                      {workOrder.assigned_to && (
                        <span>Assigned to: <span className="font-medium text-slate-900">{workOrder.assigned_to}</span></span>
                      )}
                  ...
                  {workOrder.status !== 'completed' && (
                    <button
                      onClick={() => handleStartProcedure(workOrder.id)}
