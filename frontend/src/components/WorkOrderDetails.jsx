...
function WorkOrderDetails({ workOrderId, onLogsClick, onAssignClick, onIssueEscalate }) {
  ...
  const handleStepComplete = async (stepId, workOrderId) => {
    ...
    try {
      ...
      if (response.ok) {
        // Create a log entry for step completion
        try {
          ...
          await fetch('/api/logs', {
            ...
            body: JSON.stringify({
              work_order_id: workOrderId,
              step_id: stepId,
              agent_action: `Step ${step?.step_number || ''} completed successfully`,
              result: 'Technician confirmed step completion',
              source: 'technician',
              log_type: 'success',
            }),
          })
    ...
    try {
      ...
      if (issueReport.escalateToEngineer) {
        ...
        if (response.ok) {
          ...
          // Create a log entry for escalation
          try {
            await fetch('/api/logs', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                work_order_id: workOrderId,
                step_id: step.id,
                agent_action: `Issue escalated to engineer: ${issueReport.description}`,
                result: 'Escalation message sent to engineering team',
                source: 'technician',
                log_type: 'warning',
              }),
            })
        ...
        if (response.ok) {
          ...
          // Create a log entry for issue reporting and step regeneration
          try {
            await fetch('/api/logs', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                work_order_id: workOrderId,
                step_id: step.id,
                agent_action: `Issue reported for Step ${step.step_number || issueReport.stepIndex + 1}: ${issueReport.description}`,
                result: 'Steps regenerated by AI based on issue description',
                source: 'technician',
                log_type: 'warning',
              }),
            })
